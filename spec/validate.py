...
def validate_line(obj: Any, i: int, out: list, *, first: bool) -> None:
    ...
    if ev == "session":
        ...
        runtimes = [k for k in ("python", "node", "dotnet", "go", "java", "php") if k in obj]
        if len(runtimes) != 1:
            out.append(f"line {i}: session must name exactly one runtime (python|node|dotnet|go|java|php), got {runtimes}")
        return
