...
| `python` \| `node` \| `dotnet` | string | the runtime version. Exactly one, naming the runtime that produced the tape. Adding a runtime name is additive (a new key readers ignore); the conformance checkers carry the recognized set. |
