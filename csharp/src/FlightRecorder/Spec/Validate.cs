...
namespace FlightRecorder.Spec
{
    public static class Validate
    {
        ...
        private static readonly string[] Runtimes = { "python", "node", "dotnet", "go", "java", "php" };
        ...
        private static void ValidateLine(object? obj, int i, List<string> outp, bool first)
        {
            ...
            if (ev == "session")
            {
                ...
                if (runtimes.Count != 1)
                    outp.Add($"line {i}: session must name exactly one runtime (python|node|dotnet|go|java|php), got [{string.Join(",", runtimes)}]");
                return;
            }
